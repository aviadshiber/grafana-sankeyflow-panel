...
describe('parsePanelData', () => {
  it('auto-detects edge fields across frames and aggregates duplicates', () => {
    const result = parsePanelData({
      frames: [
        frame({ from: ['A'], to: ['B'], amount: [2] }, 'A'),
        frame({ source: ['A'], target: ['B'], value: [3] }, 'B'),
      ],
      options: options(),
    });
  ...
  it('honors explicit mappings and reports invalid input without poisoning valid rows', () => {
    ...
    expect(result.graph.diagnostics.map((item) => item.code)).toEqual(
      expect.arrayContaining(['invalid-row', 'negative-value'])
    );
  });
  ...
  it('turns nullable path stages into links between nearest present stages with scoped IDs', () => {
    ...
    const result = parsePanelData({
      frames: [frame({ first: ['A'], middle: [null], last: ['A'], weight: [4] })],
      options: config,
    });
  ...
  it.each([
    ...
    const result = parsePanelData({
      frames: [frame({ source: ['A', 'A', 'A'], target: ['B', 'B', 'B'], value: [1, 5, 3] })],
      options: config,
    });
  ...
  it('applies minimumValue after duplicate aggregation and then applies graph caps', () => {
    ...
    expect(result.graph.diagnostics).toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'limit-exceeded' })])
    );
  });
    ...
    expect(result.graph.diagnostics).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          code: 'limit-exceeded',
          message: expect.stringContaining('before graph construction'),
          scope: 'global',
        }),
      ])
    );
    expect(result.frames.flatMap((item) => item.graph.diagnostics)).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ scope: 'global' })])
    );
  });
  ...
  it('enforces ingestion and diagnostic ceilings with explicit diagnostics', () => {
    ...
    const invalidRows = parsePanelData({
      frames: [
        frame({
          source: Array.from({ length: 1_001 }, () => ''),
          target: Array.from({ length: 1_001 }, () => 'B'),
          value: Array.from({ length: 1_001 }, () => 1),
        }),
      ],
      options: options(),
    });
    ...
    expect(invalidRows.graph.diagnostics.at(-1)).toEqual(
      expect.objectContaining({
        code: 'limit-exceeded',
  ...
  it('never emits non-finite link or node values for arbitrary numeric inputs', () => {
    fc.assert(
      fc.property(fc.array(fc.oneof(fc.double(), fc.constant(null)), { minLength: 1, maxLength: 40 }), (values) => {
        const result = parsePanelData({
          frames: [
            frame({
              source: values.map((_, index) => `s${index}`),
              target: values.map((_, index) => `t${index}`),
              value: values,
            }),
          ],
          options: options(),
