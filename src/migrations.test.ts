import { migrateSankeyFlowOptions } from './migrations';
...
describe('migrateSankeyFlowOptions', () => {
  it('deep-merges defaults, filters unknown data, and preserves current nested values', () => {
    const saved = {
      schemaVersion: 0,
      dataMode: 'paths',
      edgeFields: { source: 'from', tooltipFields: ['region'] },
    ...
    expect(migrated).toEqual({
      ...defaultOptions,
      dataMode: 'paths',
      edgeFields: { ...defaultOptions.edgeFields, source: 'from', tooltipFields: ['region'] },
